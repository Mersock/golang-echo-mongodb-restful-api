...
func findProducts(ctx context.Context, q url.Values, collection dbiface.CollectionAPI) ([]Product, *echo.HTTPError) {
	...
	if id, ok := filter["_id"]; ok {
		...
		if err != nil {
			...
			return products, echo.NewHTTPError(http.StatusInternalServerError, errorMessage{Message: "Unable to convert to ObjectID"})
		}
		filter["_id"] = docID
	}
	...
	if err != nil {
		...
		return products, echo.NewHTTPError(http.StatusNotFound, errorMessage{Message: "Unable to find the product"})
	}

	err = cursor.All(ctx, &products)
	if err != nil {
		log.Errorf("Unable to read the cursor :%v", err)
		return products, echo.NewHTTPError(http.StatusInternalServerError, errorMessage{Message: "Unable to read the cursor"})
	}
...
func insertProducts(ctx context.Context, products []Product, collection dbiface.CollectionAPI) ([]interface{}, *echo.HTTPError) {
	...
	for _, product := range products {
		...
		if err != nil {
			...
			return nil, echo.NewHTTPError(http.StatusInternalServerError, errorMessage{Message: "Unable to insert"})
		}
...
func (h *ProductHandlers) CreateProducts(c echo.Context) error {
	...
	if err := c.Bind(&products); err != nil {
		...
		return echo.NewHTTPError(http.StatusBadRequest, errorMessage{Message: "Unable to parse request payload"})
	}
...
func modifyProduct(ctx context.Context, id string, reqBody io.ReadCloser, collection dbiface.CollectionAPI) (Product, *echo.HTTPError) {
	...
	if err != nil {
		...
		return product, echo.NewHTTPError(http.StatusInternalServerError, errorMessage{Message: "Unable to convert to ObjectID"})
	}
	filter := bson.M{"_id": docId}
	res := collection.FindOne(ctx, filter)
	if err := res.Decode(&product); err != nil {
		log.Errorf("Unable to decode to product: %v", err)
		return product, echo.NewHTTPError(http.StatusNotFound, errorMessage{Message: "Unable to find the product"})
	}

	if err := json.NewDecoder(reqBody).Decode(&product); err != nil {
		log.Errorf("Unable to decode to reqBody: %v", err)
		return product, echo.NewHTTPError(http.StatusBadRequest, errorMessage{Message: "Unable to parse request body"})
	}

	if err := v.Struct(product); err != nil {
		log.Errorf("Unable to validate of struct: %v", err)
		return product, echo.NewHTTPError(http.StatusBadRequest, errorMessage{Message: "Unable to validate the request body"})
	}
...
func deleteProduct(ctx context.Context, id string, collection dbiface.CollectionAPI) (int64, *echo.HTTPError) {
	...
	if err != nil {
		...
		return 0, echo.NewHTTPError(http.StatusInternalServerError, errorMessage{Message: "Unable to parse request payload"})
	}
	res, err := collection.DeleteOne(ctx, bson.M{"_id": docId})
	if err != nil {
		log.Errorf("Unable to delete the product :%v", err)
		return 0, echo.NewHTTPError(http.StatusBadRequest, errorMessage{Message: "Unable to delete the product"})
	}
